...
[![Container Repository on Quay](https://quay.io/repository/samdoran/rhel6-ansible/status "Container Repository on Quay")](https://quay.io/repository/samdoran/rhel6-ansible)
...
In order to build a new image, set `RHSM_USERNAME`, `RHSM_PASSWORD`, and `RHSM_POOL_ID` as environment variables, or pass them in directly to the the `podman build` command.
    ...
    podman build -t rhel6-ansible --build-arg RHSM_USERNAME=$RHSM_USERNAME --build-arg RHSM_PASSWORD=$RHSM_PASSWORD --build-arg RHSM_POOL_ID=$RHSM_POOL_ID -f Containerfile .
